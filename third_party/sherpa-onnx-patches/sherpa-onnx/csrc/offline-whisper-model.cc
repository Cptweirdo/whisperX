...
class OfflineWhisperModel::Impl {
 public:
  ...
  std::pair<Ort::Value, Ort::Value> GetInitialSelfKVCache(int32_t batch = 1) {
    std::array<int64_t, 4> shape{n_text_layer_, batch, n_text_ctx_,
                                 n_text_state_};
...
std::pair<Ort::Value, Ort::Value> OfflineWhisperModel::GetInitialSelfKVCache(
    int32_t n) const {
  return impl_->GetInitialSelfKVCache(n);
}
