...
class OfflineRecognizerWhisperImpl : public OfflineRecognizerImpl {
 public:
  ...
  void DecodeStreams(OfflineStream **ss, int32_t n) const override {
    // whisperX batched-decode patch: one encoder pass + lockstep greedy decode
    // for the whole set. Falls back to the sequential loop for n == 1, for the
    // timestamp modes (per-stream attention state), and for auto language
    // detection (per-row detection is not batched).
    const auto &w = config_.model_config.whisper;
    if (n > 1 && !w.enable_token_timestamps && !w.enable_segment_timestamps &&
        !w.language.empty()) {
      if (DecodeStreamsBatch(ss, n)) {
        return;
      }
      // Batched decode failed (e.g. out-of-memory at this batch size) — the
      // sequential loop below is the safe retry.
    }
 ...
 private:
  // whisperX batched-decode patch: stack every stream's normalized mel into one
  // (N, T, C) tensor, run a single ForwardEncoder, then lockstep batched greedy
  // decode. Returns false if the ORT run failed (caller retries sequentially).
  bool DecodeStreamsBatch(OfflineStream **ss, int32_t n) const {
    decoder_->SetConfig(config_.model_config.whisper);

    constexpr int32_t kMaxNumFrames = 3000;
    const int32_t feat_dim = ss[0]->FeatureDim();
    ...
    std::vector<std::vector<float>> frames(n);
    std::vector<int32_t> num_frames(n);
    int32_t max_frames = 0;
    for (int32_t i = 0; i != n; ++i) {
      frames[i] = ss[i]->GetFrames();
      int32_t nf = static_cast<int32_t>(frames[i].size()) / feat_dim;
      if (nf >= kMaxNumFrames - 50) {
        SHERPA_ONNX_LOGE(
            "Only waves less than 30 seconds are supported. We process only "
            "the first 30 seconds and discard the remaining data");
        nf = kMaxNumFrames - 50;
      }
      model_->NormalizeFeatures(frames[i].data(), nf, feat_dim);
      num_frames[i] = nf;
      max_frames = std::max(max_frames, nf);
    }

    // One common T for the batch: the longest row plus tail padding (the same
    // padding DecodeStream applies per stream), capped at the 30 s window.
    const int32_t actual_frames =
        std::min(max_frames + tail_padding_frames, kMaxNumFrames);

    std::array<int64_t, 3> shape{n, actual_frames, feat_dim};

    Ort::Value mel = Ort::Value::CreateTensor<float>(
        model_->Allocator(), shape.data(), shape.size());
    float *p_mel = mel.GetTensorMutableData<float>();
    for (int32_t i = 0; i != n; ++i) {
      float *dst = p_mel + static_cast<int64_t>(i) * actual_frames * feat_dim;
      std::copy(frames[i].data(),
                frames[i].data() +
                    static_cast<int64_t>(num_frames[i]) * feat_dim,
                dst);
      std::fill_n(dst + static_cast<int64_t>(num_frames[i]) * feat_dim,
                  static_cast<int64_t>(actual_frames - num_frames[i]) *
                      feat_dim,
                  0.0f);
    }
    ...
    try {
      ...
      // Init() only ever constructs the greedy decoder (anything else exits),
      // so the downcast is safe.
      auto *greedy =
          static_cast<OfflineWhisperGreedySearchDecoder *>(decoder_.get());
      auto results = greedy->DecodeBatch(
          std::move(cross_kv.first), std::move(cross_kv.second), num_frames);
      ...
      SHERPA_ONNX_LOGE(
          "\n\nCaught exception in batched decode (batch=%d, frames=%d):"
          "\n\n%s\n\nFalling back to sequential decoding.",
          n, actual_frames, ex.what());
      return false;
    }
