...
// whisperX batched-decode patch. Lockstep greedy decode over N encoder
// outputs: one ForwardDecoder per step for the whole batch, so the per-token
// kernel-launch overhead is amortized N ways. All rows share the offset (the
// decoder graph slices positional embeddings by offset[0], and lockstep keeps
// every row at the same step). Rows that hit EOT (or their per-row token cap)
// keep being fed EOT so tensor shapes stay rectangular; their outputs are
// ignored. Mirrors Decode()'s plain-text semantics — no language detection
// (caller guarantees config_.language), no timestamp modes, no attention.
std::vector<OfflineWhisperDecoderResult>
OfflineWhisperGreedySearchDecoder::DecodeBatch(
    Ort::Value cross_k, Ort::Value cross_v,
    const std::vector<int32_t> &num_feature_frames) {
  auto memory_info =
      Ort::MemoryInfo::CreateCpu(OrtDeviceAllocator, OrtMemTypeDefault);

  const int32_t batch = static_cast<int32_t>(num_feature_frames.size());

  // Identical initial-token rows: [sot, lang, task, no_timestamps] for
  // multilingual models, [sot, no_timestamps] otherwise.
  std::vector<int64_t> initial_tokens = model_->GetInitialTokens();

  if (model_->IsMultiLingual()) {
    const auto &lang2id = model_->GetLang2ID();
    if (config_.language.empty() || !lang2id.count(config_.language)) {
      SHERPA_ONNX_LOGE("DecodeBatch requires a valid fixed language; got '%s'",
                       config_.language.c_str());
      SHERPA_ONNX_EXIT(-1);
    }
    initial_tokens[1] = lang2id.at(config_.language);
  ...
  const int32_t num_initial = static_cast<int32_t>(initial_tokens.size());
  std::vector<int64_t> token_rows(static_cast<std::size_t>(batch) *
                                  num_initial);
  for (int32_t b = 0; b != batch; ++b) {
    std::copy(initial_tokens.begin(), initial_tokens.end(),
              token_rows.begin() + static_cast<std::size_t>(b) * num_initial);
  }

  std::array<int64_t, 2> token_shape{batch, num_initial};
  Ort::Value tokens = Ort::Value::CreateTensor(
      memory_info, token_rows.data(), token_rows.size(), token_shape.data(),
      token_shape.size());
  ...
  *(offset.GetTensorMutableData<int64_t>()) = 0;
  ...
  auto self_kv_cache = model_->GetInitialSelfKVCache(batch);
  ...
  *(std::get<5>(decoder_out).GetTensorMutableData<int64_t>()) = num_initial;

  auto logits_shape =
      std::get<0>(decoder_out).GetTensorTypeAndShapeInfo().GetShape();
  const int32_t vocab_size = static_cast<int32_t>(logits_shape[2]);

  const int32_t n_text_ctx = model_->TextCtx();
  const int32_t eot = model_->EOT();

  // Current argmax per row — from the last position of the initial forward
  // (logits: (batch, num_initial, vocab)).
  std::vector<int32_t> cur(batch);
  {
    const float *p_logits = std::get<0>(decoder_out).GetTensorData<float>();
    const int64_t t = logits_shape[1];
    for (int32_t b = 0; b != batch; ++b) {
      const float *row = p_logits + (b * t + (t - 1)) * vocab_size;
      cur[b] = MaxElementIndex(row, vocab_size);
    }
  }

  // Per-row token caps, mirroring Decode(): at most 6 tokens per second of
  // audio, bounded by half the text context.
  std::vector<int32_t> max_tokens(batch);
  int32_t max_steps = 0;
  for (int32_t b = 0; b != batch; ++b) {
    int32_t cap = static_cast<int32_t>(num_feature_frames[b] / 100.0 * 6);
    cap = std::min<int32_t>(cap, n_text_ctx / 2);
    max_tokens[b] = cap;
    max_steps = std::max(max_steps, cap);
  }

  std::vector<std::vector<int32_t>> predicted(batch);
  ...
  for (int32_t i = 0; i != max_steps; ++i) {
    ...
    for (int32_t b = 0; b != batch; ++b) {
      if (done[b]) continue;
      if (cur[b] == eot || i >= max_tokens[b]) {
        done[b] = true;
        continue;
      }
      predicted[b].push_back(cur[b]);
      all_done = false;
    }
    if (all_done) {
      break;
    ...
    std::array<int64_t, 2> step_shape{batch, 1};
    Ort::Value step_tokens = Ort::Value::CreateTensor<int64_t>(
        model_->Allocator(), step_shape.data(), step_shape.size());
    int64_t *p_tokens = step_tokens.GetTensorMutableData<int64_t>();
    for (int32_t b = 0; b != batch; ++b) {
      p_tokens[b] = done[b] ? eot : cur[b];
    }

    decoder_out = model_->ForwardDecoder(std::move(step_tokens),
                                         std::move(std::get<1>(decoder_out)),
                                         std::move(std::get<2>(decoder_out)),
                                         std::move(std::get<3>(decoder_out)),
                                         std::move(std::get<4>(decoder_out)),
    ...
    *p_offset += 1;
    ...
    for (int32_t b = 0; b != batch; ++b) {
      if (!done[b]) {
        cur[b] = MaxElementIndex(p_logits + b * vocab_size, vocab_size);
      }
    }
  }

  std::vector<OfflineWhisperDecoderResult> ans(batch);
  ...
  for (int32_t b = 0; b != batch; ++b) {
    ans[b].lang = lang;
    ans[b].tokens = std::move(predicted[b]);
  }
